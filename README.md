<!-- ANIMATED WAVE HEADER -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,40:1a1035,100:7c3aed&height=220&section=header&text=Muhammad%20Sufyan%20Khan&fontSize=44&fontColor=ffffff&animation=fadeIn&fontAlignY=36&desc=.NET%20Backend%20Developer%20%7C%20SQL%20Engineer%20%7C%20API%20Specialist&descAlignY=58&descSize=17&descColor=c4b5fd" />
</div>

<!-- TYPING ANIMATION -->
<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=21&pause=1200&color=A78BFA&center=true&vCenter=true&width=700&height=50&lines=Building+scalable+.NET+backend+systems+%F0%9F%94%A7;ASP.NET+Core+%7C+C%23+%7C+SQL+Server+%E2%9A%99%EF%B8%8F;JWT+Auth+%7C+Clean+Architecture+%7C+SOLID+%F0%9F%94%90;Turning+complex+problems+into+clean+code+%F0%9F%9A%80;Enterprise-grade+software+solutions+%F0%9F%8E%AF" alt="Typing SVG" />
</div>

<br/>

<!-- STATUS BADGES -->
<div align="center">
  <img src="https://img.shields.io/badge/Open%20to%20Work-.NET%20Backend-7c3aed?style=for-the-badge&logo=dotnet&logoColor=white" />
  <img src="https://img.shields.io/badge/Location-Pakistan-22c55e?style=for-the-badge&logo=googlemaps&logoColor=white" />
  <img src="https://img.shields.io/badge/Focus-Backend%20%26%20APIs-0ea5e9?style=for-the-badge&logo=swagger&logoColor=white" />
</div>

<br/>

---

## 🧑‍💻 About Me — In C#

```csharp
namespace MuhammadSufyan.Profile
{
    public class Developer
    {
        public string Name         => "Muhammad Sufyan Khan";
        public string Title        => ".NET Backend Developer | SQL Engineer | API Specialist";
        public string Education    => "Computer Science Undergraduate 🎓";
        public string Location     => "Pakistan 🇵🇰";
        public string Email        => "khansufyanasim@gmail.com";

        public string[] CurrentWork => new[]
        {
            "🔭 Backend APIs & Final Year Project",
            "🌱 Advanced ASP.NET Core, Microservices & Cloud",
            "🧠 AI-powered systems for security & automation"
        };

        public string[] Strengths => new[]
        {
            "High-performance REST APIs",
            "JWT Auth & Role-Based Security",
            "Clean Architecture & SOLID Principles",
            "Entity Framework Core & LINQ"
        };

        public string Motto => "Secure systems. Clean code. Continuous growth. 🚀";
    }
}
```

---

## 🛠️ Tech Stack

<div align="center">

<br/>

**💻 Backend & Languages**

<br/>

[![Skills](https://skillicons.dev/icons?i=cs,dotnet,python&theme=dark&perline=6)](https://skillicons.dev)

<br/><br/>

**🗄️ Databases & ORM**

<br/>

[![Skills](https://skillicons.dev/icons?i=mssql,mysql&theme=dark&perline=6)](https://skillicons.dev)

<br/><br/>

**🔐 Security & Auth**

<br/>

![JWT](https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white)
![Role Based Auth](https://img.shields.io/badge/Role--Based%20Auth-7c3aed?style=for-the-badge&logoColor=white)
![Secure APIs](https://img.shields.io/badge/Secure%20APIs-22c55e?style=for-the-badge&logoColor=white)

<br/><br/>

**⚒️ Tools & DevOps**

<br/>

[![Skills](https://skillicons.dev/icons?i=visualstudio,vscode,git,github,postman,docker&theme=dark&perline=6)](https://skillicons.dev)

<br/>

![Swagger](https://img.shields.io/badge/Swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=black)

</div>

---

## 🚀 Featured Projects

<div align="center">

| 🏷️ Project | 📋 Description | 🔧 Stack |
|:-----------|:--------------|:---------|
| 🚖 **[Cab Management System](https://github.com/MuhammadSufyanKhn/Cab-Manegement-System-in-.NET)** | Full-stack cab booking & fleet management system | `ASP.NET MVC` `C#` `SQL Server` |
| ✅ **[Task Management System](https://github.com/MuhammadSufyanKhn/Web-Based-Task-Management-System)** | Web-based task tracker with user roles & management | `ASP.NET` `HTML` `SQL Server` |
| 🤖 **[CodeLensAI](https://github.com/MuhammadSufyanKhn/CodeLensAI)** | AI-powered code analysis and intelligent review tool | `Python` `AI/ML` |
| 🌐 **[My Portfolio](https://github.com/MuhammadSufyanKhn/My-Portfolio)** | Personal developer portfolio website | `HTML` `CSS` `JS` |
| 🧠 **[Nexus Agent Prototype](https://github.com/MuhammadSufyanKhn/Nexus-Agent-Prototype)** | Autonomous AI agent prototype | `Python` `AI` |

</div>

---

## 🌐 Connect With Me

<div align="center">

[![LinkedIn](https://skillicons.dev/icons?i=linkedin&theme=dark)](https://www.linkedin.com/in/muhammad-sufyan-khan-72574b3a3/)
&nbsp;
[![Gmail](https://skillicons.dev/icons?i=gmail&theme=dark)](mailto:khansufyanasim@gmail.com)
&nbsp;
[![GitHub](https://skillicons.dev/icons?i=github&theme=dark)](https://github.com/MuhammadSufyanKhn)

</div>

<br/>

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=MuhammadSufyanKhn&label=Profile+Views&color=7c3aed&style=for-the-badge" />
</div>

<br/>

<!-- ANIMATED WAVE FOOTER -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:7c3aed,100:0d1117&height=130&section=footer" />
</div>
